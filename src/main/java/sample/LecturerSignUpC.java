...
import sample.dao.LecturerDaoImp;

import java.net.URL;
import java.util.ResourceBundle;

public class LecturerSignUpC implements Initializable {
    ...
    {
        if(SemCombo.getItems ().isEmpty () || SubCombo.getItems ().isEmpty () || Name.getText ().isEmpty () || ID.getText ().isEmpty ())
        {
        ...
        else
        {
            ...
            {
                if(Lecturer.LecturersList.get (i).Id.equals (ID.getText ()))
                {
                    flagId=false;
                }
            ...
            for(int i=0;i<Lecturer.LecturersList.size ();i++)
            {
                if(Lecturer.LecturersList.get (i).GetNameOfSub ()==SubCombo.getSelectionModel ().getSelectedItem ());
                {
                    flagSub=false;
                }
            }
            ...
            else
            {
                ...
                if(flag)
                {
                    ...
                    Lecturer lecturer = new Lecturer (Name.getText (),ID.getText (),lecturerSubject);

                    LecturerSubject.LecturerSubjects.add (lecturerSubject);
                    Lecturer.LecturersList.add (lecturer);

                    LecturerDaoImp.save (lecturer);
                    LSubjectDaoImp.save (lecturerSubject);
                }
                else
                {
                    Lecturer lecturer=new Lecturer (Name.getText (),ID.getText (),LecturerSubject.LecturerSubjects.get (x));
                    Lecturer.LecturersList.add (lecturer);
                    LecturerDaoImp.save (lecturer);
                }
